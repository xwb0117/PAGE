...
from models.kp_util import IOStream
...
class KP_Estimator(nn.Module):
    def __init__(self, args, device):
        super(KP_Estimator, self).__init__()
        self.args = args
        self.k = args.k
        self.output_channels= args.keypointsNo*3
        self.keypointsNo = args.keypointsNo
        self.device = device
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(128*2, 256, kernel_size=1, bias=False),
                                   self.bn4,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv5 = nn.Sequential(nn.Conv1d(480, args.emb_dims, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
    ...
    def forward(self, x):
        ...
        x = self.dp2(x)
        x = self.linear3(x)
        x = torch.reshape(x,(batch_size,self.keypointsNo,3))
        return x
